...
respectively.
 ...
 $info = $dhb->{mysql_dbd_stats}

DBD::mysql keeps track of a few statistics in the mysql_dbd_stats attribute.
The following statistics are being maintained:
...
to mysql if the connection be lost. This feature defaults to on for backwards
compatibility; however, if LOCK TABLE be used, this feature should be turned
off because table locks will be lost when DBD::mysql reconnects to the server.
This attribute is ignored when AutoCommit is turned off, and DBD::mysql will
