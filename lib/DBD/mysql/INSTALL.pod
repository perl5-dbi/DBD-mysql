...
=head1 ENABLE ENCRYPTED CONNECTIONS via SSL

Connecting to your servers over an encrypted connection (SSL) is only possible
if you enabled this setting at build time. This is easily
accomplished by providing the C<--ssl> option to Makefile.PL.
Alternatively, you can also set an environment variable for this:
...
The environment variable is also honored on Windows systems.  Set the proper
environment variable and then install L<DBD::mysql>.
...
To solve this issue, you need to set the library path, similar to LD_LIBRARY_PATH on other Unix variants, buit on OS X you need to do the following (this is for a binary install of MySQL from Oracle)
